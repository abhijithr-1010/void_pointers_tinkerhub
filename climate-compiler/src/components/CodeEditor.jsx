...
export default function CodeEditor({ climate, onKeystroke, slowInput, chaosLevel }) {
  ...
  const handleKeyDown = (e) => { if (inputBlockedRef.current) { e.preventDefault(); return; } onKeystroke?.(); };
  ...
  return (
    <div className="flex rounded-lg overflow-hidden border text-xs font-mono flex-1"
      style={{ backgroundColor: climate.palette.editorBg, borderColor: climate.palette.border, minHeight: '320px' }}>
      <div className="flex flex-col items-end px-3 pt-4 select-none border-r"
        style={{ backgroundColor: climate.palette.editorBg, borderColor: climate.palette.border, color: climate.palette.border, minWidth: '3rem' }}>
