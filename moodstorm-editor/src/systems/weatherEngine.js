...
//  maps WMO codes to climate states, and manages auto-shift timer.
// ============================================================

import { CLIMATES, WMO_TO_CLIMATE } from './climateConfig.js';

// 20 cities with lat/lon for random city selection
export const CITIES = [
...
/**
 * Fetch real weather from Open-Meteo and return { climate, city, temp, description }
 * Falls back to a random local profile if the fetch fails.
 */
export async function fetchWeather(city) {
  ...
  try {
    ...
    const climate = WMO_TO_CLIMATE[wmoCode] ?? pickRandomClimate();
    return {
      climate,
      city: target,
    ...
    // Graceful fallback — no network or API unavailable
    return {
...
/**
 * Returns a random auto-shift interval in ms (3–5 minutes)
 */
