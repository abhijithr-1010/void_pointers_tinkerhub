// CompileButton.jsx — fake run button + weather-styled progress bar
...
export default function CompileButton({ climate, onCompileComplete }) {
  ...
  const handleCompile = async () => {
    ...
    const sequence = getCompileSequence(climate.compileStyle);
  ...
  const barStyle = {
    drip: {
      background: `linear-gradient(180deg, ${climate.palette.progressBar}cc, ${climate.palette.progressBar}44)`,
      animation: 'drip 1.2s ease-in infinite',
    },
    sunbeam: {
      background: `linear-gradient(90deg, ${climate.palette.progressBar}, #fff9, ${climate.palette.progressBar})`,
      backgroundSize: '200% 100%',
    },
    freeze: {
      background: `linear-gradient(90deg, ${climate.palette.progressBar}88, ${climate.palette.progressBar})`,
      transition: 'width 1.5s ease-out',
    },
    glitch: {
      background: climate.palette.progressBar,
    ...
    <div className="flex flex-col gap-2">
      <button
        ...
        style={{
          ...
          boxShadow: compiling ? 'none' : `0 0 16px ${climate.palette.glowColor}`,
          '--glow-color': climate.palette.glowColor,
        }}
      >
      ...
      {compiling && label && (
        <p className="text-xs font-mono opacity-70 italic" style={{ color: climate.palette.text }}>
          {label}
        </p>
      )}
    </div>
